 ...
 */
export const contain3OrMoreVowels = (word: string): boolean => {
 ...
 */
export const containDoubleLetter = (word: string): boolean => [ ...word ].some((
	letter: string,
	index: number,
	array: string[]
 ...
 */
export const containForbiddenStrings = (word: string): boolean => [ ...forbiddenStrings ].some((
 ...
 */
export const containPairOfTwoLetters = (word: string): boolean => [ ...word ].some((
	letter: string,
	index: number
): boolean => {
	if (index > 0) {
		const letterPair = `${word[index - 1]}${letter}`;
		return word.indexOf(letterPair, index + 1) > -1;
	}
 ...
 */
export const containRepeatingLetterWithOneLetterBetween = (word: string): boolean => [ ...word ].some((
	letter: string,
	index: number,
	array: string[]
): boolean => {
	if (index > 1) return letter === array[index - 2];
	return false;
});